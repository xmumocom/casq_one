...
public class SimScenario implements Serializable {
	...
	protected void createHosts() {
		...
		for (int i=1; i<=nrofGroups; i++) {
			...
			for (int j=0; j<nrofHosts; j++) {
				...
				//通过gid的值判断该DTNHost是否为RSU节点
				//根据配置文件设定若groupID包含r则说明是RSU,设置类型为1
				if(gid.contains("r"))      host.setType(1);    
				hosts.add(host);		
			}
			//如果节点类型是RSU：1,则将该节点添加到edges中
			edges=new ArrayList<DTNHost>();
			for(DTNHost dedge:hosts){
				if(dedge.getType()==1) edges.add(dedge);
			}
		}
	}
	...
	/*
	 * Return the list of edge nodes for this scenario 
	 */
	public List<DTNHost> getEdges(){
		return this.edges;
	}
