 ...
 * be configured.
 */
public class RequestMessageEventGenerator implements EventQueue {
	 ...
	 */
	public RequestMessageEventGenerator(Settings s){
...
//		int timeDiff = (int) (Math.sqrt(0.2)*random.nextGaussian()+1);
	...
	public ExternalEvent nextEvent() {
		int responseSize = 10; /* zero stands for one way messages */
		int msgSize;
		int interval;
		int from;
		int to;
		...
		//为查询消息创建消息生成事件，设置eventType为1表示该事件是查询事件
		MessageCreateEvent mce = new MessageCreateEvent(from, to, this.getID(), 
				msgSize, responseSize, this.nextEventsTime,Message.Query_Type);
