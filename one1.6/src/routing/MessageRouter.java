...
//		Message retuMessa=new Message(m.getTo(), m.getFrom(), null, msgTtl);
//		retuMessa.addProperty("response1", "This is a response message");
//		if(createNewMessage(retuMessa)) System.out.println("success!!");
	...
	public Collection<Application> getApplications(String ID) {
		...
		if (ID != null) {
			tmp = (List<Application>) this.applications.get(null);
			if (tmp != null) {
				apps.addAll(tmp);
			}
		}
